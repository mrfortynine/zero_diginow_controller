...
size_t HardwareBLESerial::print(int64_t value) {
  char buf[21]; snprintf(buf, 21, "%d", value); // the longest representation of a uint64_t is for -2^63, 20 characters plus null terminator
  return this->print(buf);
}
...
size_t HardwareBLESerial::print(uint64_t value) {
  char buf[21]; snprintf(buf, 21, "%u", value);  // the longest representation of a uint64_t is for 2^64-1, 20 characters plus null terminator
  return this->print(buf);
}
